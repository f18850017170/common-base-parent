 ...
 * im账号签名生成工具类
 * 来自官方材料
 * 理论上 一个账户的签名可以存在多份，只要各自的过期时间不过期，就能使用
 */
public class TlsSignatureUtils {
	public static class GenTLSSignatureResult {
		public String errMessage;
		public String urlSig;
		public int expireTime;//单位：秒
		public int initTime;//创建时间 时间戳/1000
		private long initTimestamps;//创建时的时间戳
		private long expireTimestamps;//过期时的时间戳
	...
	public static GenTLSSignatureResult GenTLSSignature(long expire,
		...
		try {
			...
			String userSig = new String(
					Base64UrlUtils.base64EncodeUrl(Arrays.copyOfRange(compressBytes, 0, compressBytesLength)));
	...
	public static CheckTLSSignatureResult CheckTLSSignature(String urlSig,
		...
		byte[] compressBytes = Base64UrlUtils.base64DecodeUrl(urlSig.getBytes(Charset.forName("UTF-8")));
	...
	public static GenTLSSignatureResult GenTLSSignatureEx(
		...
		result.initTime = Integer.parseInt(time);
		result.expireTime = (int) expire;
		result.setInitTimestamps(currentTimeMillis);
		result.setExpireTimestamps(new DateTime(currentTimeMillis).plusSeconds((int) expire).toDate().getTime());
		String SerialString =
				"TLS.appid_at_3rd:" + 0 + "\n" +
						"TLS.account_type:" + 0 + "\n" +
						"TLS.identifier:" + identifier + "\n" +
